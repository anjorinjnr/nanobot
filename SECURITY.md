...
**WhatsApp:**
- The WhatsApp client (neonize / whatsmeow Go bindings) runs in-process — no separate
  network listener and no IPC token to manage
- Keep authentication data in `~/.nanobot/whatsapp-auth` secure (mode 0700) — it
  contains the WhatsApp Web session sqlite DB and is sufficient to impersonate the account
...
- WhatsApp: in-process neonize client, no network listener, no IPC token
