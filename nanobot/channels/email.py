...
class EmailConfig(Base):
    ...
    # approved guests).  Generated by build_context.py.  If empty, all
    # senders in allow_from are routed to the agent.
    known_senders_file: str = ""
    # Path to a JSONL file for logging unknown senders.  A heartbeat task
    # reads this to escalate to the user.
    unknown_sender_log: str = ""
...
class EmailChannel(BaseChannel):
    ...
    async def _fetch_and_dispatch(self) -> None:
        ...
        Pre-filters unknown senders before they reach the LLM.  Known
        senders (in allow_from) are routed normally; unknown senders are
        ...
        for item in inbound_items:
            ...
            # senders are logged (no LLM cost) for heartbeat escalation.
            if not self._is_known_sender(sender):
                self._log_unknown_sender(item)
                continue
    ...
    def _is_known_sender(self, sender: str) -> bool:
        ...
        Returns True (allow) if no known_senders_file is configured.
        Reloads the file when its mtime changes.
        """
        ks_path = self.config.known_senders_file
        if not ks_path:
        ...
        try:
            ...
            return True  # File missing — don't block anyone

        if self._known_senders_cache is None or mtime != self._known_senders_mtime:
            import json as _json
            try:
                data = _json.loads(path.read_text(encoding="utf-8"))
                self._known_senders_cache = {e.strip().lower() for e in data if isinstance(e, str)}
    ...
    def _log_unknown_sender(self, item: dict[str, Any]) -> None:
        ...
        Does nothing if ``unknown_sender_log`` is not configured.
        """
        log_path = self.config.unknown_sender_log
        if not log_path:
            return
        import json as _json
        entry = {
        ...
        try:
            ...
            logger.info("Email from unknown sender {} logged (subject: {})", entry["sender"], entry["subject"])
        except Exception as e:
            logger.warning("Failed to log unknown email sender: {}", e)
